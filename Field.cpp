...
Field::Field(int newHeight, int newWidth) : player(new Player()) {
	...
	if (newHeight < 0) 
		lessZeroHeight = "the field is initialized with a negative height";
	if (newWidth < 0)
		lessZeroWidth = "the field is initialized with a negative width";
...
void Field::addObserver(Observer* observer) {
	observers.push_back(observer);
	Notify("The game is on", "Field", TRACE);
	if (lessZeroHeight != "") 
		Notify(lessZeroHeight, "Field", WARN);
	if (lessZeroWidth != "")
		Notify(lessZeroWidth, "Field", WARN);
}
